...
#define PERIOD_US         20000u       /* 20 ms -> 50 Hz servo window */
#define PULSE_MIN_US      1000u
#define PULSE_MAX_US      2000u
#define PULSE_CENTER_US   1500u

static void busy_wait_us(unsigned us)
{
    unsigned chunks = (us * 100u) / CYCLES_PER_CHUNK;
    unsigned i;
    for (i = 0; i < chunks; i++)
...
{
    __R30 |= SERVO_BIT_PIN;            /* Pin HIGH instantly */
    busy_wait_us(width_us);
    __R30 &= ~SERVO_BIT_PIN;           /* Pin LOW instantly */
...
int main(void)
{
    ...
    // No OCP enables or GPIO register setup needed anymore!
    while (1) {
